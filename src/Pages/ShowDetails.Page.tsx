...
const ShowDetailPage: FC<ShowDetailPageProps> = ({ params, getShowDetails, showDetail, getShowCast, showCast, castLoading, showDetailLoading}) => {
  useEffect(()=> {
  ...
  if(!showDetail){
    return <div className="font-bold text-7xl flex justify-center items-center h-screen"><LoadingSpinner /></div>
  }
  ...
  return (
    <div className="mt-2">
      ...
      <div className="flex my-2 bg-gray-300 p-2 rounded-sm justify-between">
        ...
        {showDetail.genres.length ? showDetail.genres.map(g => <GenrePill name={g} />) : "Genres Not Avilable"}
        </div>
        {showDetailLoading  && <div className="flex items-center font-thin text-purple-900"> Loading<LoadingSpinner /> </div>}
      </div>
      <div className="mt-2 flex">
        <img
          src={showDetail.image?.original || "https://t3.ftcdn.net/jpg/02/48/42/64/360_F_248426448_NVKLywWqArG2ADUxDq6QprtIzsF82dMF.jpg"}
          alt=""
